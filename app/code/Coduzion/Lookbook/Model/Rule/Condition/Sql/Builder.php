...
use Coduzion\Lookbook\Model\Rule\Condition\Lookbook;
...
class Builder
{
    ...
    protected function _getMappedSqlCondition(
        ...
        if ($condition->getInputType() === 'string'
            ...
            $mapcondition = $this->_conditionOperatorMap[$conditionOperator];
            $bindValue = $condition->getBindArgumentValue();
            if (stripos($mapcondition, ':field IN (?)') !== false && $bindValue) {
     ...
     * @return void
     */
    private function buildConditions(Collection $collection, Combine $combine) : void
    {
