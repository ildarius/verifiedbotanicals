...
(function($){
    ...
    $.fn.easypinShow = function(options) {
        ...
        try {
            ...
            var depends = {
                ...
                pin: options.pin || 'marker-pin.svg',
                data: options.data || {},
        ...
        $(depends.allCanvas).each(function(i) {
            ...
            if(typeof(depends.data[parentId]) != 'undefined') {
                ...
                for (var j in depends.data[parentId]) {
                    ...
                    var productInfo = depends.data[parentId][j];

                    // Display product information in the popover
                    if (productInfo && productInfo.product_id) {

                        var productId = productInfo.product_id;  
                        var productName = productInfo.name;
                        var productImgUrl = productInfo.imgurl;
                        var productPrice = productInfo.price;
                        var productPageUrl = productInfo.url;
                        
                        var popoverContent = '<div class="easypinpopup">';
                            popoverContent += '<a href="' + productPageUrl + '">';
                            popoverContent += '<img src="' + productImgUrl + '" />';
                            popoverContent += '<div class="easypinpopupinfo">';
                            popoverContent += '<h3>' + productName + '</h3>';
                            popoverContent += '<p>' + productPrice + '</p>';
                            popoverContent += '</div>';
                        ...
                        if (depends.data[parentId][j].content) {
                            depends.data[parentId][j].content += popoverContent;
                        } else {
                            depends.data[parentId][j].content = popoverContent;
                        }

                        // Update the product information for the current pin
                        depends.data[parentId][j]['imgurl'] = productImgUrl;
                        depends.data[parentId][j]['name'] = productName;
                        depends.data[parentId][j]['price'] = productPrice;
                        depends.data[parentId][j]['url'] = productPageUrl;                            
                    }
                    ...
                    var viewContainer = viewLocater(depends.data[parentId], j, parentWidth, parentHeight, createView(returnData, tpl));
                    ...
                    if(depends.popover.show == true) {
                        ...
                        $('.easypin-marker', pinContainer).addClass('active-pin');
                    }
                    ...
                    // popover
                    $('.easypin-marker:last', pinContainer).on('mouseenter', function(e) {

                        $('.easypin-popover').hide();
                        $('.easypin-marker').removeClass('active-pin');
                        ...
                        var marker = $(this);
                        var popover = $('.easypin-popover', marker);
                        ...
                        var spaceTop = markerTop;
                        var spaceBottom = containerHeight - (markerTop + markerHeight);
                        var spaceLeft = markerLeft;
                        var spaceRight = containerWidth - (markerLeft + markerWidth);

                         // Determine if there's more space at the top or bottom
                        var moreVerticalSpaceOnTop = spaceTop > spaceBottom;

                        // Position the popover based on available space
                        popover.css({
                            top: moreVerticalSpaceOnTop ? 'auto' : '100%',
                            bottom: moreVerticalSpaceOnTop ? '100%' : 'auto',
                            left: '50%',
                        ...
                        if(depends.popover.animate === true) {
                            ...
                            $('.easypin-popover', ins).toggle(500);
                            ...
                            if ($('.easypin-popover', this).css('display') == 'none' ){
                               // $('.easypin-popover', this).show();
                                $('.easypin-popover', this).show();
                                $('.easypin-marker', this).addClass('active-pin');
                            }else{
                                //$('.easypin-popover', this).hide();
                                $('.easypin-popover', this).hide();
                                $('.easypin-marker', this).removeClass('active-pin');
                            }
                        }
    ...
    var viewLocater = function(data, markerIndex, parentWidth, parentHeight, markerContainer) {
        ...
        var pos = calculatePinRate(data[markerIndex],pinWidth, markerWidth, pinHeight, markerHeight, parentWidth, parentHeight);
    ...
    var calculatePinRate = function(data, pinWidth, markerWidth, pinHeight, markerHeight, parentWidth, parentHeight) {
        var lat = parseInt(data.coords.lat);
        var long = parseInt(data.coords.long);
        var newlat = (parentWidth * lat) / pinWidth;
        var newlong = (parentHeight * long) / pinHeight;

        return {
            left: (newlat/parentWidth)*100,
            top: ((newlong-(markerHeight))/parentHeight)*100,
        };
    };
